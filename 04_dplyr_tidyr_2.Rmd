...
If you noticed above, `conc` was still a character after we pivoted. We want to make some graphs, so we need it to be a number. There is a way to do this inside the call to `pivot`, at this point it's easier to use what we already know.

### mini-challenge  

What `dplyr` function do you want to use in the following code chunk, to turn `conc` into a number?  
