...
This has a lot of rows. We might only be interested in observations about a single bird species, or from a single state, which means we want to choose rows. To do this, we use the verb `filter()`. The data argument comes first, then the condition.    
...
Let's go ahead and assign it to a data frame so we can explore it a little more.  
