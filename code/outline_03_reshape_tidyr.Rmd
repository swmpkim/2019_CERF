...
```{r}
tox <- -----(here::here("data", "C_elegans_tox.csv"), stringsAsFactors = FALSE) 
...
Using the ebird dataset, filter the data frame to only include data from the `year` 2012; remove the `samplesize` column; then pivot so that each `state` is a column header, each row is a species, and the value in each cell is `presence` for that state/species combination.  
